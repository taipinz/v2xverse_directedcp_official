...
class V2XVERSEBaseDataset(Dataset):
    def __init__(self, params, visualize, train=True):
        ...
        if 'target_assigner_config' in self.params['loss']['args']:
            self.det_range = self.params['loss']['args']['target_assigner_config'][
                'cav_lidar_range']  # [-36, -36, -22, 36, 36, 14]
        else:
            ...
            if len(rsu_files) > 0:
                for rsu_file in ['rsu_1000', 'rsu_1001']:
                    if rsu_file in rsu_files:
                        rsu_measurements_path = os.path.join(route_path, rsu_file, 'measurements')
                        if os.path.exists(rsu_measurements_path):
                            rsu_frame_len = len(os.listdir(rsu_measurements_path))
                            file_len_list.append(rsu_frame_len)

            # 添加空列表检查
            if not file_len_list:
                file_len_list = [0]  # 确保列表不为空
    ...
    def _load_json(self, path):
        ...
        try:
            ...
            _logger.info(f"Failed to load {path}: {e}")

        # 尝试多个前序帧
        n = path[-9:-5]
        for offset in range(1, 6):  # 尝试前1到前5帧
            try:
                new_path = path[:-9] + "%04d.json" % (int(n) - offset)
                _logger.info(f"Trying alternative path with offset {offset}: {new_path}")
                json_value = json.load(open(os.path.join(self.root_dir, new_path)))
                _logger.info(f"Successfully loaded alternative frame: {new_path}")
                return json_value
            except Exception as e2:
                _logger.info(f"Also failed to load alternative {new_path}: {e2}")
                continue

        # 如果所有尝试都失败，返回空字典
        _logger.warning(f"All attempts failed for {path}, returning empty dict")
        return {}
    ...
    def get_one_record(self, route_dir, frame_id, agent='ego', visible_actors=None, tpe='all', extra_source=None):
        ...
        # =========================================================
        # [修改] 仅保留车辆 (Car only filter)
        # =========================================================
        actors_data_clean = {}
        for aid, ainfo in actors_data.items():
            # 0: Car, 1: Ped/Bike, 2: Light.
            # 我们只保留 0。这样模型就变成了单分类训练。
            if ainfo.get('tpe') == 0:
                actors_data_clean[aid] = ainfo
        actors_data = actors_data_clean
        # =========================================================
        ...
        for cam_id in range(4):
            ...
            output_record['params']['camera{}'.format(cam_id)]['cords'] = [measurements['x'], measurements['y'], 1.0, \
                                                                           0, measurements['theta'] / np.pi * 180 +
                                                                           cam_angle_list[cam_id], 0]
            output_record['params']['camera{}'.format(cam_id)]['extrinsic'] = measurements[
                'camera_{}_extrinsics'.format(cam_list[cam_id])]
            output_record['params']['camera{}'.format(cam_id)]['intrinsic'] = measurements[
                'camera_{}_intrinsics'.format(cam_list[cam_id])]
        ...
        output_record['params']['map_pose'] = \
            [measurements['lidar_pose_x'] + self.distance_to_map_center * np.cos(measurements["theta"] - np.pi / 2),
             ...
             0, measurements['theta'] / np.pi * 180 - 90, 0]
        detmap_pose_x = measurements['lidar_pose_x'] + self.distance_to_map_center * np.cos(
            measurements["theta"] - np.pi / 2)
        detmap_pose_y = measurements['lidar_pose_y'] + self.distance_to_map_center * np.sin(
            measurements["theta"] - np.pi / 2)
        ...
        if tpe == 'all':
            if route_dir is not None:
                ...
                output_record['rgb_front'] = self._load_image(
                ...
                output_record['rgb_right'] = self._load_image(
        ...
        for actor_id in actors_data.keys():
            ...
            if not ('box' in actors_data[actor_id].keys() and 'ori' in actors_data[actor_id].keys() and 'loc' in
                    actors_data[actor_id].keys()):
                continue
            ...
            output_record['params']['vehicles'][actor_id]['location'] = [actors_data[actor_id]['loc'][0],
                                                                         actors_data[actor_id]['loc'][1], 0]
            output_record['params']['vehicles'][actor_id]['speed'] = 3.6 * math.sqrt(
                actors_data[actor_id]['vel'][0] ** 2 + actors_data[actor_id]['vel'][1] ** 2)
        ...
        for i, direction in enumerate(direction_list):
            ...
            output_record['params']['camera{}'.format(i)]['cords'] = \
                [measurements['x'] + dis_to_lidar * np.sin(measurements['theta']),
                 measurements['y'] - dis_to_lidar * np.cos(measurements['theta']), 2.3, \
                 0, measurements['theta'] / np.pi * 180 - 90 + theta_list[i], 0]
            output_record['params']['camera{}'.format(i)]['extrinsic'] = measurements[
                'camera_{}_extrinsics'.format(direction_list[i])]
            output_record['params']['camera{}'.format(i)]['intrinsic'] = measurements[
                'camera_{}_intrinsics'.format(direction_list[i])]
    ...
    def retrieve_base_data(self, idx, tpe='all', extra_source=None, data_dir=None):
        if extra_source is None:
            ...
            data = OrderedDict()
            data['car_0'] = self.get_one_record(scene_dict['ego'], frame_id, agent='ego',
                                                visible_actors=visible_actors['car_0'], tpe=tpe)
            if self.params['train_params']['max_cav'] > 1:
                for i, route_dir in enumerate(scene_dict['other_egos']):
                    try:
                        data['car_{}'.format(i + 1)] = self.get_one_record(route_dir, frame_id_latency,
                                                                           agent='other_ego',
                                                                           visible_actors=visible_actors[
                                                                               'car_{}'.format(i + 1)], tpe=tpe)
                    except:
            ...
            if self.params['train_params']['max_cav'] > 2:
                for i, rsu_dir in enumerate(scene_dict['rsu']):
                    try:
                        data['rsu_{}'.format(i)] = self.get_one_record(rsu_dir, frame_id_latency, agent='rsu',
                                                                       visible_actors=visible_actors[
                                                                           'rsu_{}'.format(i)], tpe=tpe)
                    except:
            ...
            scene_dict = None
            frame_id = None
            data['car_0'] = self.get_one_record(route_dir=None, frame_id=None, agent='ego', visible_actors=None,
                                                tpe=tpe, extra_source=extra_source['car_data'][0])
            if self.params['train_params']['max_cav'] > 1:
                if len(extra_source['car_data']) > 1:
                    for i in range(len(extra_source['car_data']) - 1):
                        data['car_{}'.format(i + 1)] = self.get_one_record(route_dir=None, frame_id=None,
                                                                           agent='other_ego', visible_actors=None,
                                                                           tpe=tpe,
                                                                           extra_source=extra_source['car_data'][i + 1])
                for i in range(len(extra_source['rsu_data'])):
                    data['rsu_{}'.format(i)] = self.get_one_record(route_dir=None, frame_id=None, agent='rsu',
                                                                   visible_actors=None, tpe=tpe,
                                                                   extra_source=extra_source['rsu_data'][i])
...
def generate_det_data(
    ...
    traffic_heatmap = traffic_heatmap[:int((det_range[0] + det_range[1]) / res),
                      int((max_distance - det_range[2]) / res):int((max_distance + det_range[3]) / res)]
    det_data = np.zeros(
        (int((det_range[0] + det_range[1]) / res), int((det_range[2] + det_range[3]) / res), 7))  # (50,25,7)
